...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]) {
	for (int i = 0; i < num; i++) {
		if ((bufX[i] >= save_minX) && (bufX[i] <= save_maxX) && (bufY[i] >= save_minY) && (bufY[i] <= save_maxY)) {
		  ...
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX)+1, 32 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX)+1, 33 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX)+1, 31 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX), 33 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX), 31 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX)-1, 33 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX)-1, 32 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		  ST7735_DrawPixel((127 * (bufX[i] - save_minX)) / (save_maxX - save_minX)-1, 31 + (127 * (save_maxY - bufY[i])) / (save_maxY - save_minY), ST7735_BLUE);
		}
